...
std::string UnitAction::toString() const {
    return std::string("UnitAction") + "(vel:" +
        std::to_string(velocity) + " jump:" +
        (jump ? "true" : "false") +  " jumpdown:" +
        (jumpDown ? "true" : "false") + " aim:" +
        aim.toString() + " shoot:" +
        (shoot ? "true" : "false") +  " reload:" +
        (reload ? "true" : "false") + " swap:" +
